  ...
  try {
    ...
    if (env.OPENAI_API_KEY) {
      try {
        ...
        if (!/quota|billing|insufficient/i.test(error.message)) throw error;
        return json({ answer: localProjection(question, snapshot, `AI quota 문제: ${error.message}`) });
      }
    }
    if (env.GEMINI_API_KEY) {
      try {
        ...
        if (!/quota|billing|할당량|free_tier/i.test(error.message)) throw error;
        return json({ answer: localProjection(question, snapshot, `AI quota 문제: ${error.message}`) });
      }
    }
    return json({ answer: localProjection(question, snapshot, "AI 키가 없어 내장 계산 모드로 답합니다.") });
  } catch (error) {
    return json({ answer: `AI 답변 실패: ${error.message}` }, { status: 500 });
  }
}
